...
public class FoodConsumedService : IFoodConsumedService
{
    ...
    public FoodConsumedService(IUnitOfWork unitOfWork, IMapper mapper)
    {
        this.unitOfWork = unitOfWork;
        this.mapper = mapper;
    }

    public async Task<List<models.FoodConsumed>> GetFoodConsumedAsync(int userId, DateTime date, CancellationToken cancellationToken)
    {
        var foodConsumedEntities = await unitOfWork.FoodConsumedRepository
        ...
        var foodConsumedGrouped = foodConsumedEntities
            ...
            .Select(g =>
            {
                ...
                var foo = new models.FoodConsumed
                {
                    ...
                    EquivalentUnits = GetEquivalentUnits(g.Key, totalQuantity, totalCookedQuantity, totalPracticalQuantity, date.Date)
                };

                return foo;
            });

        var foodConsumed = foodConsumedGrouped.ToList();

        //Round Quantities to 2 decimal places
        foodConsumed.ForEach(fc =>
        {
